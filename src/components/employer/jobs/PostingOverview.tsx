...
// Overview tab: the read-only JD card (status, apply URL, details, description)
// with a pencil escape hatch into the existing PostingForm editor and Close
// posting as the destructive action at the bottom. Extracted from
// DetailSettings — behaviour unchanged, only the tab it lives on.
      ...
      <Stack gap={18}>
        <Stack gap={8} dir="row" align="center" wrap>
          ...
          {allowEdit && (
            <span style={{ marginLeft: 'auto' }}>
              <Button variant="ghost" size="sm" aria-label="Edit posting" onClick={() => setMode('edit')}>
                <Pencil size={15} />
              </Button>
            </span>
          )}
        ...
        <div>
          ...
          {posting.status === 'active' ? (
            <Stack gap={8} dir="row" align="center" wrap>
              <code style={{ fontSize: '0.875rem', color: 'var(--ink)' }}>{applyUrl}</code>
